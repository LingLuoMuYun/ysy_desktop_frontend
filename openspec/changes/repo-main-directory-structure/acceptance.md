...
- [x] 顶层目录结构文件均已创建。
- [x] README 包含目标仓库结构说明。
- [x] AGENTS 指向 `apps/frontend/`、`apps/backend/`、`apps/agent/` 和 `packages/shared/`。
...
```text
Test-Path apps/frontend, apps/backend, apps/agent, packages/shared, scripts, tests, configs, docs/architecture: all present through README files.
...
git status: old root frontend/, output/ and PRD PDFs remain untracked and unstaged.
```
